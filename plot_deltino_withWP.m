...
dt = 1/5; % sampling time for plotting [s]
...
reached_pp = interp1(tout, reached.', time).';
reached_pp(end) = reached_pp(end)+1;
...
for i=1:steps2
    ...
    Pa11(:,i) = T_SB1 * R_z(alpha11) * [R_A, 0, 0, 1].';
    Pa12(:,i) = T_SB1 * R_z(alpha12) * [R_A, 0, 0, 1].';
    Pa13(:,i) = T_SB1 * R_z(alpha13) * [R_A, 0, 0, 1].';
end 
...
for i = 1:steps2
    T_SB1 = T_ypr(ypr_pp(1,i), ypr_pp(2,i), ypr_pp(3,i), base1(1,i), base1(2,i), base1(3,i));
    Pc11(:,i) = T_SB1 * R_z(alpha11) * [R_A+l_A*cos(theta1_pp(1,i)), 0, -l_A*sin(theta1_pp(1,i)), 1].';
    Pc12(:,i) = T_SB1 * R_z(alpha12) * [R_A+l_A*cos(theta1_pp(2,i)), 0, -l_A*sin(theta1_pp(2,i)), 1].';
    Pc13(:,i) = T_SB1 * R_z(alpha13) * [R_A+l_A*cos(theta1_pp(3,i)), 0, -l_A*sin(theta1_pp(3,i)), 1].';
end
...
for i = 1:steps2
    ...
    Pb11(:,i) = P1(:,i) + R_SB * [R_B*cos(alpha11), R_B*sin(alpha11), 0, 1].' - [0;0;0;1];
    Pb12(:,i) = P1(:,i) + R_SB * [R_B*cos(alpha12), R_B*sin(alpha12), 0, 1].' - [0;0;0;1];
    Pb13(:,i) = P1(:,i) + R_SB * [R_B*cos(alpha13), R_B*sin(alpha13), 0, 1].' - [0;0;0;1];
end
...
for i=1:steps2
    ...
    Pa21(:,i) = T_SB2 * R_z(alpha21) * [R_A, 0, 0, 1].';
    Pa22(:,i) = T_SB2 * R_z(alpha22) * [R_A, 0, 0, 1].';
    Pa23(:,i) = T_SB2 * R_z(alpha23) * [R_A, 0, 0, 1].';
end 
% Knees coordinates in frame {S}
Pc21 = zeros(4,steps2);
Pc22 = zeros(4,steps2);
...
for i = 1:steps2
    T_SB2 = T_ypr(ypr_pp(1,i), ypr_pp(2,i), ypr_pp(3,i), base2(1,i), base2(2,i), base2(3,i));
    Pc21(:,i) = T_SB2 * R_z(alpha21) * [R_A+l_A*cos(theta2_pp(1,i)), 0, -l_A*sin(theta2_pp(1,i)), 1].';
    Pc22(:,i) = T_SB2 * R_z(alpha22) * [R_A+l_A*cos(theta2_pp(2,i)), 0, -l_A*sin(theta2_pp(2,i)), 1].';
    Pc23(:,i) = T_SB2 * R_z(alpha23) * [R_A+l_A*cos(theta2_pp(3,i)), 0, -l_A*sin(theta2_pp(3,i)), 1].';
end
% Foot coordinates in frame {S}
P2 = zeros(4,steps2);
for i=1:steps2
...
for i = 1:steps2
    R_SB = T_ypr(ypr_pp(1,i), ypr_pp(2,i), ypr_pp(3,i), 0, 0, 0);
    Pb21(:,i) = P2(:,i) + R_SB * [R_B*cos(alpha21), R_B*sin(alpha21), 0, 1].' - [0;0;0;1];
    Pb22(:,i) = P2(:,i) + R_SB * [R_B*cos(alpha22), R_B*sin(alpha22), 0, 1].' - [0;0;0;1];
    Pb23(:,i) = P2(:,i) + R_SB * [R_B*cos(alpha23), R_B*sin(alpha23), 0, 1].' - [0;0;0;1];
end
...
for i=1:steps2
    ...
    Pa31(:,i) = T_SB3 * R_z(alpha31) * [R_A, 0, 0, 1].';
    Pa32(:,i) = T_SB3 * R_z(alpha32) * [R_A, 0, 0, 1].';
    Pa33(:,i) = T_SB3 * R_z(alpha33) * [R_A, 0, 0, 1].';
end 
% Knees coordinates in frame {S}
Pc31 = zeros(4,steps2);
Pc32 = zeros(4,steps2);
...
for i = 1:steps2
    T_SB3 = T_ypr(ypr_pp(1,i), ypr_pp(2,i), ypr_pp(3,i), base3(1,i), base3(2,i), base3(3,i));
    Pc31(:,i) = T_SB3 * R_z(alpha31) * [R_A+l_A*cos(theta3_pp(1,i)), 0, -l_A*sin(theta3_pp(1,i)), 1].';
    Pc32(:,i) = T_SB3 * R_z(alpha32) * [R_A+l_A*cos(theta3_pp(2,i)), 0, -l_A*sin(theta3_pp(2,i)), 1].';
    Pc33(:,i) = T_SB3 * R_z(alpha33) * [R_A+l_A*cos(theta3_pp(3,i)), 0, -l_A*sin(theta3_pp(3,i)), 1].';
end
% Foot coordinates in frame {S}
P3 = zeros(4,steps2);
for i=1:steps2
...
for i = 1:steps2
    R_SB = T_ypr(ypr_pp(1,i), ypr_pp(2,i), ypr_pp(3,i), 0, 0, 0);
    Pb31(:,i) = P3(:,i) + R_SB * [R_B*cos(alpha31), R_B*sin(alpha31), 0, 1].' - [0;0;0;1];
    Pb32(:,i) = P3(:,i) + R_SB * [R_B*cos(alpha32), R_B*sin(alpha32), 0, 1].' - [0;0;0;1];
    Pb33(:,i) = P3(:,i) + R_SB * [R_B*cos(alpha33), R_B*sin(alpha33), 0, 1].' - [0;0;0;1];
end
...
for i=1:steps2
    ...
    Pa41(:,i) = T_SB4 * R_z(alpha41) * [R_A, 0, 0, 1].';
    Pa42(:,i) = T_SB4 * R_z(alpha42) * [R_A, 0, 0, 1].';
    Pa43(:,i) = T_SB4 * R_z(alpha43) * [R_A, 0, 0, 1].';
end 
% Knees coordinates in frame {S}
Pc41 = zeros(4,steps2);
Pc42 = zeros(4,steps2);
...
for i = 1:steps2
    T_SB4 = T_ypr(ypr_pp(1,i), ypr_pp(2,i), ypr_pp(3,i), base4(1,i), base4(2,i), base4(3,i));
    Pc41(:,i) = T_SB4 * R_z(alpha41) * [R_A+l_A*cos(theta4_pp(1,i)), 0, -l_A*sin(theta4_pp(1,i)), 1].';
    Pc42(:,i) = T_SB4 * R_z(alpha42) * [R_A+l_A*cos(theta4_pp(2,i)), 0, -l_A*sin(theta4_pp(2,i)), 1].';
    Pc43(:,i) = T_SB4 * R_z(alpha43) * [R_A+l_A*cos(theta4_pp(3,i)), 0, -l_A*sin(theta4_pp(3,i)), 1].';
end
% Foot coordinates in frame {S}
P4 = zeros(4,steps2);
for i=1:steps2
...
for i = 1:steps2
    R_SB = T_ypr(ypr_pp(1,i), ypr_pp(2,i), ypr_pp(3,i), 0, 0, 0);
    Pb41(:,i) = P4(:,i) + R_SB * [R_B*cos(alpha41), R_B*sin(alpha41), 0, 1].' - [0;0;0;1];
    Pb42(:,i) = P4(:,i) + R_SB * [R_B*cos(alpha42), R_B*sin(alpha42), 0, 1].' - [0;0;0;1];
    Pb43(:,i) = P4(:,i) + R_SB * [R_B*cos(alpha43), R_B*sin(alpha43), 0, 1].' - [0;0;0;1];
end
...
% Plot waypoints
numWP = size(WPs,1)-1;
waypoints = gobjects(numWP,1);
waypoints(1) = plot3(WPs(1,1), WPs(1,2), 0, 'h', 'Color', 'blue', ...
	'MarkerFaceColor', 'cyan', 'MarkerSize', 15);	
teta=-pi:0.01:pi;
x0=radius0*cos(teta)+WPs(1,1);
y0=radius0*sin(teta)+WPs(1,2);
circle0 = plot3(x0,y0,zeros(1,numel(x0)), 'cyan', 'Linewidth', 1);
x1=radius1*cos(teta)+WPs(1,1);
y1=radius1*sin(teta)+WPs(1,2);
circle1 = plot3(x1,y1,zeros(1,numel(x0)), 'red', 'LineStyle','--');
for idx = 2:numWP
	waypoints(idx) = plot3(WPs(idx,1), WPs(idx,2), 0, 'h', 'Color', 'red',...
		'MarkerFaceColor', '#D95319', 'MarkerSize', 15);
end
...
for i = 1:steps2
    ...
    % Update waypoints color
	if (i>=2) && (reached_pp(i) > reached_pp(i-1))
		old_reached = old_reached +1;
		waypoints(old_reached).Color = 'black';
		waypoints(old_reached).MarkerFaceColor = 'green';
        delete([circle0, circle1]);
		if old_reached < numWP 
			new_wp = old_reached+1;
			x0=radius0*cos(teta)+WPs(new_wp,1);
			y0=radius0*sin(teta)+WPs(new_wp,2);
			circle0 = plot3(x0,y0,zeros(1,numel(x0)),'cyan', 'Linewidth', 1);
            waypoints(new_wp).Color = 'blue';
			waypoints(new_wp).MarkerFaceColor = 'cyan';
			x1=radius1*cos(teta)+WPs(new_wp,1);
			y1=radius1*sin(teta)+WPs(new_wp,2);
			circle1 = plot3(x1,y1,zeros(1,numel(x1)),'red','LineStyle','--');
		end
	end	
    ...
    res0 = text(0, -0.05, 0, ['Time: ' num2str(time(i)) 's']);
    % Plot trunk of robot
